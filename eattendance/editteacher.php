...
<div class="container-fluid" style="background-color:#7ec6e6";>
          ...
          <h1 class="h1_index text-center">Update Teacher Details.</h1>
          <br>
...
while($post=mysqli_fetch_assoc($record)) {
   ...
   ?>
      ...
      <tr>
        ...
        <td><input type="text" name="name" class="form-control" id="formGroupExampleInput" value="<?php echo $post['id']; ?>" placeholder="id"></td></td>
...
if(isset($_POST['update'])) {
    ...
    $conn = mysqli_connect('localhost','root','','eattendance');
 ...
 if (mysqli_query($conn,$sql1))
  {
   ...
   <script>
	  ...
	  if(!alert('Teacher details updated successfully.')){window.location = "teacher.php?view_employee=view_employee";}
   </script>
   <?php
       } 
   else {
    ?>
     <script>
       // window.location = "admin.php?view_employee=view_employee";
          if(!alert("Can't update teacher details.Some error occured")){window.location = "teacher.php?view_employee=view_employee";}
